(() => {
  ...
  // src/claimDetail/claimDetailAPI.ts
  ...
  var getClaimsDetails = async (campaignId) => {
    ...
    let lockInfoList = await import_vesting_sdk.Claimant.getUnverifiedLockInfo(wallet, address, campaignId);
    let claimInfoList = await import_vesting_sdk.Claimant.getMyClaims(wallet, address, campaignId);
    let lockExtInfoList = [];
        ...
        if (lockInfoItem.dataUri) {
          ...
          for (let j = 0; j < vestingList.length; j++) {
            ...
            if (vestingItem.account == wallet.address) {
              let lockExtInfo = __spreadProps(__spreadValues({}, lockInfoItem), {
                merkleTreeData: vestingList,
                locked: import_eth_wallet.Utils.fromDecimals(vestingItem.amount, tokenObj.decimals).toFixed(),
    ...
    let claimExtInfoList = [];
    let claimable = new import_eth_wallet.BigNumber(0);
    for (let i = 0; i < claimInfoList.length; i++) {
      let claimInfoItem = claimInfoList[i];
      claimable = claimable.plus(claimInfoItem.claimable);
      claimExtInfoList.push(__spreadProps(__spreadValues({}, claimInfoItem), {
        claimed: import_eth_wallet.Utils.fromDecimals(claimInfoItem.claimed, tokenObj.decimals).toFixed(),
        claimable: import_eth_wallet.Utils.fromDecimals(claimInfoItem.claimable, tokenObj.decimals).toFixed(),
        locked: import_eth_wallet.Utils.fromDecimals(claimInfoItem.locked, tokenObj.decimals).toFixed()
      }));
    }
    let claimsDetailsObj = {
      lockInfoList: lockExtInfoList,
      claimInfoList: claimExtInfoList,
      claimable: import_eth_wallet.Utils.fromDecimals(claimable, tokenObj.decimals).toFixed(),
      tokenObj
    };
    return claimsDetailsObj;
  };
  var verifyMerkleLock = async (lockId, vestingData) => {
    let wallet = import_eth_wallet.Wallet.getInstance();
    let address = await (0, import_store.getVaultContractAddress)();
    if (!address)
      return null;
    let receipt = await import_vesting_sdk.Claimant.doVerifyMerkleLock(wallet, address, lockId, vestingData);
    return receipt;
  };
  var claim = async (id, callback, confirmationCallback) => {
    let wallet = import_eth_wallet.Wallet.getInstance();
    let address = await (0, import_store.getVaultContractAddress)();
    if (!address)
      return null;
    let receipt = await import_vesting_sdk.Claimant.doClaim(wallet, address, id);
    return receipt;
  };
  var claimAll = async (ids, callback, confirmationCallback) => {
    let wallet = import_eth_wallet.Wallet.getInstance();
    let address = await (0, import_store.getVaultContractAddress)();
    if (!address)
      return null;
    let receipt = await import_vesting_sdk.Claimant.doClaimAll(wallet, address, ids);
    return receipt;
  };
  ...
  var ClaimDetail = class extends import_components2.Module {
    ...
    async loadApiData_claimsDetails() {
      this.claimsDetails = await getClaimsDetails(this.getCampaignId());
    ...
    async onVerify(target, data) {
      ...
      verifyMerkleLock(data.id, data.merkleTreeData);
      target.caption = "Verifying";
      target.rightIcon = /* @__PURE__ */ this.$render("i-icon", {
        name: "spinner",
        spin: true,
        margin: { left: 10 },
        width: "20px",
    ...
    async onClaimAll(target) {
      ...
      await claimAll(idList, null, null);
    }
    async renderFirstPnl() {
      this.firstPnlIcon.url = import_assets.default.fullPath((0, import_store2.getTokenIconPath)(this.claimsDetails.tokenObj, (0, import_store2.getChainId)()));
      ...
      this.firstPnlVVHstackIcon.onClick = () => import_components2.application.copyToClipboard(this.firstPnlVVHstackLabel.caption || "");
      this.claimAllBtnPanel.innerHTML = "";
      let item = /* @__PURE__ */ this.$render("i-button", {
        id: "firstPnlBtn",
        caption: "Claim All",
        height: "47",
        width: "191",
        font: { size: "18px" },
        onClick: (target) => this.onClaimAll(target)
      });
    ...
    renderClaimList() {
      for (let i = 0; i < this.claimList.length; i++) {
        if (this.claimList[i].locked != "0") {
          let item = /* @__PURE__ */ this.$render("i-grid-layout", {
            templateColumns: ["auto"],
            templateRows: ["auto"],
            minHeight: "169px",
            horizontalAlignment: "center"
          }, /* @__PURE__ */ this.$render("i-stack", {
            grid: { row: 1, column: 1 },
            ...
            stack: { basis: "40%", shrink: "1", grow: "1" },
            verticalAlignment: "center",
            horizontalAlignment: "center",
            width: "100%"
          }, /* @__PURE__ */ this.$render("i-vstack", {
            ...
            caption: Number(this.claimList[i].locked).toFixed(4) + " " + this.claimsDetails.tokenObj.symbol,
            font: { name: "Montserrat", size: "25px" },
            class: "canBreak"
          })), /* @__PURE__ */ this.$render("i-vstack", {
            verticalAlignment: "center",
            horizontalAlignment: "start",
            ...
            horizontalAlignment: "start",
            verticalAlignment: "center",
            stack: { basis: "50%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-label", {
            caption: "Vesting Start: ",
            font: { name: "Montserrat", size: "16px" }
          })), /* @__PURE__ */ this.$render("i-hstack", {
            horizontalAlignment: "start",
            verticalAlignment: "center",
            stack: { basis: "50%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-label", {
            caption: (0, import_moment.default)(this.claimList[i].vestingStart * 1e3).format(DefaultDateFormat) + " " + (0, import_moment.default)(this.claimList[i].vestingStart * 1e3).format(DefaultTimeFormat),
            font: { name: "Montserrat", size: "16px" }
          }))), /* @__PURE__ */ this.$render("i-hstack", {
            width: "100%"
          }, /* @__PURE__ */ this.$render("i-hstack", {
            ...
            caption: (0, import_moment.default)(this.claimList[i].vestingEnd * 1e3).format(DefaultDateFormat) + " " + (0, import_moment.default)(this.claimList[i].vestingEnd * 1e3).format(DefaultTimeFormat),
            font: { name: "Montserrat", size: "16px" }
          })))))), /* @__PURE__ */ this.$render("i-hstack", {
            ...
            verticalAlignment: "center",
            horizontalAlignment: "center",
            stack: { basis: "60%", shrink: "1", grow: "1" },
            border: { radius: "27px", width: 2.7, color: "#A8A8A8" },
            padding: { left: "10", right: "10", top: "10", bottom: "10" },
            width: "100%"
          }, /* @__PURE__ */ this.$render("i-vstack", {
            verticalAlignment: "center",
            ...
            horizontalAlignment: "start",
            verticalAlignment: "center",
            stack: { basis: "30%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-label", {
            caption: "Claimable: ",
            font: { name: "Montserrat", size: "16px" }
          })), /* @__PURE__ */ this.$render("i-hstack", {
            ...
            caption: Number(this.claimList[i].claimable).toFixed(4) + " " + this.claimsDetails.tokenObj.symbol,
            font: { name: "Montserrat", size: "16px" },
            class: "canBreak"
          }))), /* @__PURE__ */ this.$render("i-hstack", {
            width: "min(80%, 350px)"
            ...
            caption: Number(this.claimList[i].claimed).toFixed(4) + " " + this.claimsDetails.tokenObj.symbol,
            font: { name: "Montserrat", size: "16px" },
            class: "canBreak"
          })))), /* @__PURE__ */ this.$render("i-hstack", {
            verticalAlignment: "center",
            ...
            width: 191,
            font: { size: "18px" },
            onClick: (target) => this.onClaim(target, this.claimList[i])
          })))));
      ...
      for (let i = 0; i < this.lockList.length; i++) {
        if (this.lockList[i].locked != "0") {
          let item = /* @__PURE__ */ this.$render("i-grid-layout", {
            ...
            caption: Number(this.lockList[i].locked).toFixed(4) + " " + this.claimsDetails.tokenObj.symbol,
            font: { name: "Montserrat", size: "25px" }
          })), /* @__PURE__ */ this.$render("i-vstack", {
            verticalAlignment: "center",
            horizontalAlignment: "center",
            stack: { basis: "36%", shrink: "1", grow: "1" },
            minWidth: "250px"
          }, /* @__PURE__ */ this.$render("i-hstack", {
            width: "min(80%, 350px)"
          }, /* @__PURE__ */ this.$render("i-hstack", {
            horizontalAlignment: "start",
            verticalAlignment: "center",
            stack: { basis: "45%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-label", {
            caption: "Vesting Start: ",
            font: { name: "Montserrat", size: "16px" }
          })), /* @__PURE__ */ this.$render("i-hstack", {
            horizontalAlignment: "start",
            verticalAlignment: "center",
            stack: { basis: "55%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-label", {
            caption: (0, import_moment.default)(this.lockList[i].vestingStart * 1e3).format(DefaultDateFormat) + " " + (0, import_moment.default)(this.lockList[i].vestingStart * 1e3).format(DefaultTimeFormat),
            font: { name: "Montserrat", size: "16px" }
          }))), /* @__PURE__ */ this.$render("i-hstack", {
            width: "min(80%, 350px)"
          }, /* @__PURE__ */ this.$render("i-hstack", {
            ...
            caption: (0, import_moment.default)(this.lockList[i].vestingEnd * 1e3).format(DefaultDateFormat) + " " + (0, import_moment.default)(this.lockList[i].vestingEnd * 1e3).format(DefaultTimeFormat),
            font: { name: "Montserrat", size: "16px" }
          })))), /* @__PURE__ */ this.$render("i-hstack", {
            verticalAlignment: "center",
            horizontalAlignment: "center",
            stack: { basis: "24%", shrink: "1", grow: "1" }
          }, /* @__PURE__ */ this.$render("i-button", {
            caption: "Verify",
            height: 47,
            width: 191,
            ...
            onClick: (target) => this.onVerify(target, this.lockList[i])
          }))));
          this.vestingList.append(item);
        }
      }
    ...
    render() {
      return /* @__PURE__ */ this.$render("i-panel", {
        ...
        stack: { basis: "52%", shrink: "1", grow: "1" },
        id: "firstPnlVstack",
        resizer: true,
        horizontalAlignment: "center",
        padding: { left: "10", right: "10", top: "10", bottom: "10" },
        ...
        id: "firstPnlVstackLabel1",
        font: { size: "31.25px" }
      }), /* @__PURE__ */ this.$render("i-hstack", {
        gap: 5,
        verticalAlignment: "center"
      }, /* @__PURE__ */ this.$render("i-label", {
        id: "firstPnlVVHstackLabel",
        class: "canBreak",
        font: { name: "Montserrat", size: "12.8px" }
      }), /* @__PURE__ */ this.$render("i-icon", {
        id: "firstPnlVVHstackIcon",
        class: "firstPnlVVHstackIcon",
        height: "17",
        width: "17",
        fill: "white",
        tooltip: { content: `The address has been copied`, trigger: "click" }
      })))), /* @__PURE__ */ this.$render("i-hstack", {
        stack: { basis: "25%", shrink: "1", grow: "1" },
        horizontalAlignment: "center",
        verticalAlignment: "center",
        padding: { left: "10", right: "10", top: "10", bottom: "10" },
        minWidth: "250px"
        ...
        id: "claimAllBtnPanel",
        stack: { basis: "23%", shrink: "1", grow: "1" },
        horizontalAlignment: "center",
        padding: { left: "10", right: "10", top: "10", bottom: "10" }
      }))), /* @__PURE__ */ this.$render("i-vstack", {
        id: "vestingList",
