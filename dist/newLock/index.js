(() => {
  ...
  var import_store3 = __toModule(__require("@vesting/store"));
  var import_assets = __toModule(__require("@vesting/assets"));

  // src/newLock/index.css.ts
  ...
  // src/lockDetail/lockDetailAPI.ts
  ...
  var getLocksDetails = async (campaignId) => {
    ...
    let myLocks = await import_vesting_sdk.Locker.getMyLocks(wallet, address, campaignId);
    let lockExtInfoList = [];
        ...
        if (lockInfoItem.dataUri) {
          ...
          for (let j = 0; j < lockRecordList.length; j++) {
            ...
            lockExtInfoList.push(__spreadProps(__spreadValues({}, lockInfoItem), {
              locked: import_eth_wallet.Utils.fromDecimals(lockRecordItem.amount, tokenObj.decimals).toFixed(),
  ...
  var doMerkleLock = async (campaignId, lockRecords) => {
    let wallet = import_eth_wallet.Wallet.getInstance();
    let address = await (0, import_store.getVaultContractAddress)();
    if (!address)
      return null;
  ...
  var import_vesting_sdk2 = __toModule(__require("@scom/vesting-sdk"));
  ...
  var approveERC20Max = async (token, spenderAddress, callback, confirmationCallback) => {
    ...
    let erc20 = new import_eth_wallet3.Contracts.ERC20(wallet, token.address);
  ...
  var ERC20ApprovalModel = class {
    constructor(options) {
      ...
      this.checkAllowance = async (token, inputAmount) => {
        ...
        } else if (new import_eth_wallet4.BigNumber(inputAmount).gt(allowance)) {
  ...
  var newLock = class extends import_components2.Module {
    constructor(parent, options) {
      ...
      this.onWalletConnect = async (connected) => {
        let chainId = (0, import_store3.getChainId)();
      ...
      this.getApprovalModelAction = async (options) => {
        const address = await import_store2.getVaultContractAddress();
    ...
    async onLoad() {
      ...
      await this.onSetupPage((0, import_store3.isWalletConnected)());
    ...
    unixTimeToString(unix) {
      if (typeof unix == "number") {
        return (0, import_moment.default)(unix * 1e3).format(DefaultDateFormat);
      } else {
        return unix;
      }
    }
    async loadApiData_locksDetails() {
      this.locksDetails = await getLocksDetails(this.getCampaignId());
    }
    calculateEarilestStartDateAndLastestEndDate() {
      let startDateList = [];
      let endDateList = [];
    ...
    renderBtnBar() {
      ...
      this.confirmBtn_spin.rightIcon = /* @__PURE__ */ this.$render("i-icon", {
        name: "spinner",
        spin: true,
        height: "20px",
        width: "20px"
      });
      this.approveBtn_spin.rightIcon = /* @__PURE__ */ this.$render("i-icon", {
        name: "spinner",
        spin: true,
        height: "20px",
        width: "20px"
    ...
    renderFirstPanel() {
      this.firstPnlIcon.url = import_assets.default.fullPath((0, import_store3.getTokenIconPath)(this.locksDetails.tokenObj, (0, import_store3.getChainId)()));
    ...
    addRecipient() {
      if (!this.popup_recipient_value.value || !this.popup_amount_value.value || !this.addRecipientStartDate || !this.addRecipientEndDate) {
        return;
      }
      let item = {
    ...
    async confirmAddRecipient() {
      ...
      for (let i = 0; i < this.recipientsToBeAdded.length; i++) {
        let item = {
          ...
          amount: import_eth_wallet5.Utils.toDecimals(this.recipientsToBeAdded[i].amount).toFixed(),
      ...
      this.onPayRouter();
      await doMerkleLock(campaignId, lockRecords);
    }
    enablePopupWindow() {
    ...
    backToMyCampaignDetail() {
      let id = this.getCampaignId();
      window.location.href = `#/locks/${id}`;
    }
    async showApproveBtnAndConfirmBtn() {
    ...
    async initApprovalModelAction() {
      this.approvalModelAction = await this.getApprovalModelAction({
        ...
        onPaid: async (data) => {
          import_components2.application.EventBus.dispatch(import_global.EventId.Paid);
    ...
    downloadCSV() {
      let convertUnixToDate = this.recipientsToBeAdded;
    ...
    importCSV() {
      ...
      this.recipientsToBeAdded.forEach(function(rowArray) {
    ...
    renderTable() {
      const columnData = [
        ...
        { title: "Duration", fieldName: "duration", width: "10%", sortable: true },
        {
          ...
          width: "5%",
          sortable: false,
          onRenderCell: (source, columnData2, rowData) => {
            return /* @__PURE__ */ this.$render("i-icon", {
              id: "deleteRecipientBtn",
              class: "deleteRecipientBtn",
              name: "times-circle",
              fill: "white",
              width: "20px",
              height: "20px",
              background: { color: "#F29224" },
              onClick: () => this.deleteRecipient(rowData),
        ...
        },
        {
          ...
          onRenderCell: (source, columnData2, rowData) => {
            return /* @__PURE__ */ this.$render("i-icon", {
              ...
              background: { color: "#F29224" },
              padding: { top: 2, bottom: 2, right: 2, left: 2 },
              tooltip: { content: `The data has been copied`, trigger: "click" },
              onClick: () => import_components2.application.copyToClipboard(JSON.stringify(rowData) || "")
            });
          }
        }
      ];
      this.table.columns = columnData;
      let recipientsToBeAdded_fixed = [];
      if (this.recipientsToBeAdded.length != 0) {
        for (let i = 0; i < this.recipientsToBeAdded.length; i++) {
          let item = {
    ...
    render() {
      return /* @__PURE__ */ this.$render("i-panel", {
        ...
        horizontalAlignment: "center",
        stack: { basis: "15%", shrink: "1", grow: "1" }
        ...
        stack: { basis: "75%", shrink: "1", grow: "1" },
        gap: { row: 5, column: 5 },
        padding: { left: "10", right: "10", top: "10", bottom: "10" },
        verticalAlignment: "center"
      }, /* @__PURE__ */ this.$render("i-label", {
        id: "label1",
        grid: { row: 1, column: 1 },
        font: { name: "Montserrat", size: "13.27px" }
      }), /* @__PURE__ */ this.$render("i-label", {
        id: "label2",
        grid: { row: 1, column: 2 },
        font: { name: "Montserrat", size: "13.27px" }
        ...
        width: "200px",
        height: "46px",
        border: { width: "3.45px", color: "#9C9C9C", style: "solid", radius: "11.51px" },
        verticalAlignment: "center",
        horizontalAlignment: "center",
        background: { color: "#000000" }
      }, /* @__PURE__ */ this.$render("i-label", {
        ...
        width: "170px",
        height: "44px",
        background: { color: "linear-gradient(270deg, #FF9900 0%, #FC7428 100%)" },
        font: { size: "17.64px" },
        onClick: this.enablePopupWindow
      })))), /* @__PURE__ */ this.$render("i-hstack", {
        width: "100%",
        maxWidth: "1400px",
        horizontalAlignment: "center"
        ...
        width: "200px",
        height: "50",
        background: { color: "linear-gradient(270deg, #7B7B7B 0%, #929292 100%)" },
        onClick: this.backToMyCampaignDetail
      }), /* @__PURE__ */ this.$render("i-button", {
        id: "approveBtn",
        width: "200px",
        height: "50",
        background: { color: "#34343A" },
        onClick: this.approveAddRecipient
      }), /* @__PURE__ */ this.$render("i-button", {
        id: "confirmBtn",
        width: "200px",
        height: "50",
        onClick: this.confirmAddRecipient
      }), /* @__PURE__ */ this.$render("i-button", {
        id: "approveBtn_spin",
        width: "200px",
        height: "50",
        ...
        maxWidth: "500px",
        closeIcon: { name: "times-circle", fill: "#fff" }
      }, /* @__PURE__ */ this.$render("i-grid-layout", {
        width: "100%",
        verticalAlignment: "center",
        gap: { row: 5 },
        ...
        id: "popup_title",
        font: { size: "20px", color: "rgb(252, 116, 40)" }
      })), /* @__PURE__ */ this.$render("i-label", {
        id: "popup_recipient_label",
        grid: { area: "recipient_label_area" },
        font: { name: "montserrat" }
      }), /* @__PURE__ */ this.$render("i-input", {
        id: "popup_recipient_value",
        grid: { area: "recipient_value_area" },
        font: { name: "montserrat" },
        width: "100%"
      }), /* @__PURE__ */ this.$render("i-label", {
        id: "popup_amount_label",
        grid: { area: "amount_label_area" },
        font: { name: "montserrat" }
      }), /* @__PURE__ */ this.$render("i-input", {
        id: "popup_amount_value",
        grid: { area: "amount_value_area" },
        font: { name: "montserrat" },
        width: "100%"
      }), /* @__PURE__ */ this.$render("i-label", {
        id: "popup_startD_label",
        grid: { area: "startD_label_area" },
        font: { name: "montserrat" }
      }), /* @__PURE__ */ this.$render("i-datepicker", {
        id: "popup_startD_value",
        type: "dateTime",
        grid: { area: "startD_value_area" },
        font: { name: "montserrat" },
        width: "100%",
        captionWidth: "0px"
      }), /* @__PURE__ */ this.$render("i-label", {
        id: "popup_endD_label",
        grid: { area: "endD_label_area" },
        font: { name: "montserrat" }
      }), /* @__PURE__ */ this.$render("i-datepicker", {
        id: "popup_endD_value",
        type: "dateTime",
        grid: { area: "endD_value_area" },
        font: { name: "montserrat" },
        width: "100%",
        captionWidth: "0px"
      }), /* @__PURE__ */ this.$render("i-hstack", {
